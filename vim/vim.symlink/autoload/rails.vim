...
function! s:app_lightweight_ruby_eval(ruby,...) dict abort
  ...
  let args = '-e '.s:rquote('begin; require %{rubygems***REMOVED*** rescue LoadError; end; begin; require %{active_support***REMOVED*** rescue LoadError; end; '.a:ruby)
...
function! s:Refresh(bang)
  if exists("g:rubycomplete_rails") && g:rubycomplete_rails && has("ruby") && exists('g:rubycomplete_completions')
    ...
    silent! ruby if defined?(ActiveSupport::Dependencies***REMOVED*** ActiveSupport::Dependencies.clear; elsif defined?(Dependencies***REMOVED*** Dependencies.clear; end
    if a:bang
...
function! s:BufScriptWrappers()
  ...
  command! -buffer       -nargs=1 -range=0 -complete=customlist,s:Complete_ruby Rpp           :call rails#app().runner_command(<count>==<line2>?<count>:-1,'require %{pp***REMOVED*** pp begin '.<f-args>.' end')
...
function! s:app_dbext_settings(environment) dict
  ...
  if !has_key(cache,a:environment)
    ...
    if self.has_file("config/database.yml")
      let cmdb = 'require %{yaml***REMOVED*** File.open(%q{'.self.path().'/config/database.yml}) {|f| y = YAML::load(f***REMOVED*** e = y[%{'
